...
print('  ####   #   #    ####     #     #   ## ')
print(' #   #    ###    #   #   #####   #    # ')
print(' Detect human intronic AG-gain variants ')
print(' in BP-ACC region that disrupt splicing ')
print('****************************************\n')
...
parser.add_argument("-i", "--input",
                    help="vcf file of variants with 5 mandatory fields: CHROM, POS, ID, REF, ALT")
...
filename_coding_map = 'Data_AGAIN_coding_map_'+genome_ref+'.txt'
filename_detection = 'Data_AGAIN_detection_'+genome_ref+'_'+transcript_ref+'.bed'

timestamp = str(int(time.time()))
filename = filename_vcf[0:filename_vcf.rfind('.')]
filename_bed = filename+'.bed'
filename_mapping = filename+'.mapping'
filename_output = filename+'_AGAIN_output_'+timestamp+'.csv'
...
for eachline in file_coding_map:
    ...
    transcript = item[1]
    exon_count = int(item[4])
    start_exon = int(item[5])
    start_codon = int(item[6])
...
try:
    ...
    os.system("bedtools intersect -wo -s"
              " -a " + filename_bed +
              " -b " + filename_detection +
              " > " + filename_mapping)

    file_mapping = open(filename_mapping, 'r')
    file_out = open(filename_output, 'w')
    file_out.write('CHR,POS,ID,REF,ALT,STR,TYPE,GENE,TRANSCRIPT_IVS,CANONICAL,'
    ...
    for eachline in file_mapping:
        try:
            ...
            if variant_pass2:
                ...
                # for deletion, shift AGAIN_hit by del-size to check with pre-exisited AG
                if 'del' in var_type:
                    AGAIN_hit_set_new = set()
                    for each_pos in AGAIN_hit_set:
                        if (each_pos+len(ref)-1) not in AG_hit_wt:
                ...
                if AGAIN_hit_set:
                    ...
                    output = chrom+','+str(var_pos) +','+var_id+','+ref+','+alt+','+strand+','+\
                             var_type+','+gene+','+transcript_ivs.replace(',',';')+','+canonical+','+\
